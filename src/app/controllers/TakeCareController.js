  ...
  try {
    ...
    if (!staffId || !date || !startTime || !endTime) {
      return res.render("createTakeCare", {
        ...
        errorMessage:
          "Vui lòng điền đầy đủ thông tin (nhân viên, ngày, giờ bắt đầu, giờ kết thúc).",
      });
    }
    ...
    if (
      !staffMember ||
      !["WAITER", "KITCHENSTAFF", "RESMANAGER"].includes(staffMember.role)
    ) {
      return res.render("createTakeCare", {
        layout: "layouts/mainAdmin",
        title: "Tạo TakeCare",
        ...
        errorMessage:
          "Nhân viên không hợp lệ hoặc không thuộc vai trò WAITER, KITCHENSTAFF, hoặc RESMANAGER.",
      });
    }
  ...
  try {
    ...
    takeCares.forEach((tc) => {
      console.log(
        `ID: ${tc._id}, Nhân viên: ${
          tc.staff
            ? tc.staff.firstName + " " + tc.staff.lastName
            : "Không có nhân viên"
        }`
      );
    });
...
exports.renderUpdateTakeCare = async (req, res) => {
  try {
    ...
    if (
      !takeCare ||
      String(takeCare.staff?.restaurant) !== String(req.user.restaurant)
    ) {
  ...
  try {
    ...
    if (!staffId || !date || !startTime || !endTime) {
      ...
      return res.render("updateTakeCare", {
        ...
        staffs,
        errorMessage:
          "Vui lòng điền đầy đủ thông tin (ngày, giờ bắt đầu, giờ kết thúc, nhân viên).",
      });
    }
...
exports.getStaffSchedule = async (req, res) => {
  try {
    ...
    const takeCares = await TakeCare.find({ staff: userId }).populate("staff");

    console.log(
      `Lấy thành công lịch làm của nhân viên: ${staff.firstName} ${staff.lastName}`
    );
    takeCares.forEach((tc) => {
      console.log(
        `ID: ${tc._id}, Ngày: ${tc.date}, Thời gian: ${tc.startTime} - ${tc.endTime}`
      );
    });
